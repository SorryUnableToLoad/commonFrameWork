...
import com.apiproj.utils.LoggerUtils;
...
public class RestBase {
    ...
    protected static CfAPI getCall;
...
//    protected static PropertyUtils util = new PropertyUtils(
//	    System.getProperty("user.dir") + "/config/api_config.properties");
//    protected static PropertyUtils verifyUtil = new PropertyUtils(
//	    System.getProperty("user.dir") + "/verification/api_verify.properties");
    ...
    protected static String getBaseURLforHttps(String baseurl) {
        String sBaseUrl = "https://" + baseurl;
        return sBaseUrl;
    }
    ...
    protected static void initforHttps(String baseURL, String userName, String password) {
        resetFailure();
        sBaseUrl = getBaseURLforHttps(baseURL);
        sRestCallEngine = new RestCallEngine(sBaseUrl, createRestClient(userName, password));
        verify = new Verify();
        setTestDataDir();
    }

    protected static void initProxy(String baseURL, String userName, String password,String proxyUrl,String proxyPort,String proxyUserName,String proxyPass) {

        resetFailure();
        sBaseUrl = getBaseURL(baseURL);
        sRestCallEngine = new RestCallEngine(sBaseUrl, createRestClient(userName, password,proxyUrl,proxyPort,proxyUserName,proxyPass));
        verify = new Verify();
        setTestDataDir();
    }


    public static Client createRestClient(String userName, String password,String proxyUrl,String proxyPort,String proxyUserName,String proxyPass) {
    ...
    protected static XmlPath convertToXmlPath(Response curResponse) {
        ...
        try {
            ...
            if (sLogLevel.intValue() <= Level.FINE.intValue())
                xmlPath.prettyPrint();
            takeLog.utilList.add(xmlPath.prettyPrint());
            return xmlPath;
        } catch (Throwable t) {
    ...
    protected static JsonPath convertToJsonPath(Response curResponse) {
        ...
        try {
            ...
            takeLog.utilList.add(curJsonPath.prettyPrint());
            return curJsonPath;
        } catch (Throwable t) {
            throw new JsonParseFailException("Failed to convert to JsonPath object, Response Code:  "
    ...
    public static void downLoad(CfAPI restTestCase,String fileName){
        sRestCallEngine.downLoad(restTestCase,fileName);
    }
     ...
     * @param restTestCase
     *
     * Log request in form of curl
     */
    public static void logCurl(CfAPI restTestCase){
        String curlCommand = "curl -X " +  restTestCase.getMethod() + " '" + getBaseURL() + restTestCase.getUri() ;
        String queryParam = "";
        if (restTestCase.getParameters() != null && !restTestCase.getParameters().isEmpty()) {
            for (Map.Entry<String, String> param : restTestCase.getParameters().entrySet()) {
                queryParam= queryParam+"&" + param.getKey() + "=" + param.getValue();
            }
            curlCommand = curlCommand+ "?" + queryParam;
            ...
            for ( MultivaluedMap.Entry<String, List<String>> param : restTestCase.getMultiValuedParameters().entrySet()) {
                ...
                for (String value : param.getValue()) {
                    queryParam= queryParam+"&" + param.getKey() + "=" + value;
                }
                curlCommand = curlCommand+ "?" + queryParam;
            }
        }
        curlCommand = curlCommand +"'";
        if (restTestCase.getHeaders() != null && !restTestCase.getHeaders().isEmpty()) {
            for ( MultivaluedMap.Entry<String, List<Object>> header : restTestCase.getHeaders().entrySet()) {
                System.out.println((header.getKey() + " = " + header.getValue()));
                for (Object value : header.getValue()) {
                    curlCommand = curlCommand+" -H " + "'" + header.getKey() + ": " + value + "'";
                }
            }
        }
        if(restTestCase.getSource()!=null && !((String) restTestCase.getSource()).isEmpty()){
            curlCommand = curlCommand + " -d '" + restTestCase.getSource() + "'";
        }
        LoggerUtils.info(curlCommand);
    }
