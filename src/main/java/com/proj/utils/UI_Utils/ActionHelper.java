 ...
 */
public class ActionHelper extends WebSetUp {
...
/*protected void sendKeys(MobileElement element,String value) {
        ...
        LogStatus.pass("Screenshot below", TestUtils.pullScreenshotPath());
        element.click();
        LogStatus.pass("Clicking is successfully on "+ element);
    }*/
    ...
    private static void waitForElementToBeClickableAndClick(WebElement element) {
    ...
    public static void sendKeys(WebElement element, String s) {
        ...
        element.sendKeys(s);
        logger.info(element + " Key Sent to an element");
    }

    public static void sendKeys(MobileElement element, String dataTextValue) {
        waitForPresenceOfElement(element);
        element.sendKeys(dataTextValue);
        logger.info(element + " Key Sent to an element");
    }
    ...
    public static void sendKeys(By by, String s) {
        ...
        logger.info(by + " Key Sent to an element");
    }
    ...
    public static boolean verifyIsElementDisplayed(MobileElement element) {
        waitForPresenceOfElement(element);
        return element.isDisplayed();
    }
    ...
    public static void waitForPresenceOfElement(MobileElement element) {
        WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), getConfig().waitTime());
        wait.until(ExpectedConditions.visibilityOf(element));
    }
    ...
    /**
     * This method is performed bottom to top swipe action
     * based on int argument that is number of times
     */
    ...
    public static void performScrollTopToBottom() {
        new TouchAction((PerformsTouchActions) DriverManager.getDriver())
                ...
                .waitAction(WaitOptions.waitOptions(Duration.ofMillis(500)))
                .moveTo(PointOption.point(center, startY))
                .release()
                .perform();
    }
    ...
    public void longPress(MobileElement mobileElement, int milliSeconds) {
        new TouchAction((PerformsTouchActions) DriverManager.getDriver())
                .longPress(LongPressOptions.longPressOptions().withElement(ElementOption.element(mobileElement)))
                .waitAction(WaitOptions.waitOptions(Duration.ofMillis(milliSeconds))).perform();
    }
